...
export default function BreadcrumbSection() {
	...
	const handlePayment = (order_amount) => {
		...
		axios.post(`http://localhost:8080/razorpay/order`, amount_obj)
			.then((order) => {
		...
		<div className="fugu--breadcrumbs-section">
			<div className="fugu--breadcrumbs-data">
				...
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
					...
					<div >
						<button onClick={() => handlePayment(300)} style={{ color: "white", padding: "12px 28px", backgroundColor: "red", cursor: "pointer", fontWeight: "bold", borderRadius: "10px", fontSize: "23px" }}>&#8377; 300</button>
					</div>
				</div>
			</div>
		</div>
