...
export default function PricingSectionTwo(props) {
	...
	const { curUser, setupdationState, updationState, activePlan } = useContext(AuthContext);
	...
	const MemberPostInSanity = (CurUserIdNow, plan_amount, plan_ref, plan_days) => {
		let date = new Date();
	...
	return (
		<div className="section fugu-section-padding3">
			<div className="container">
				...
				<div className="fugu-section-title">
					{activePlan ?
						<h4>Active Plan : {activePlan.planDetail.planTitle} Valid Till   {format(new Date(activePlan.endtDate), "dd MMMM yyyy")} </h4>
						:
						null
					}
